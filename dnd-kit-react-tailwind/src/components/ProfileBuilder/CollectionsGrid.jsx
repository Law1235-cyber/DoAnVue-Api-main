...
export default function CollectionsGrid({ content, viewMode }) {
  ...
  const getGridCols = () => {
      if (viewMode === 'mobile') return 'grid-cols-1'; // Stack 1 per row
      if (viewMode === 'tablet') return 'grid-cols-2'; // 2 per row
      return 'grid-cols-4'; // 4 per row (desktop)
  };
      ...
      <div className={classNames(
        ...
        {collections.map((col, idx) => (
            <div key={idx} className="group cursor-pointer flex flex-col h-full">
                <div className={`${col.color} aspect-[4/3] w-full rounded-xl mb-3 flex items-center justify-center transition-transform duration-300 group-hover:scale-[1.02] group-hover:shadow-md relative overflow-hidden`}>
